...
//Initialisation
var app = angular.module("Jdm7dvOpenSource", ["ngResource"]);

/* ----------------------------------------------------------------------------
                Main Controller */

//Get Adobe Github repos & orgs
app.factory("jdm7dvData", function($resource) {
    return $resource("//github.com/jdm7dv");
});

//Offline backup of json
app.factory("jdm7dvDataOffline", function($resource) {
...
this.GitHubCtrl = function($scope, $sce, $filter, jdm7dvData, jdm7dvDataOffline, FeaturedHeader) {
    ...
    $scope.loadOffline = function() {
        ...
        jdm7dvDataOffline.query(function(rep) {
            if (rep[0]) {
                $scope.updateData(rep[0]);
            } else {
    ...
    $scope.updateGraph = function() {
        ...
        langsDim  = langsData.dimension(function(d) {
        return d.name;
            });
        langSumGroup = langsDim.group().reduceSum(function(d){ 
        return d.value;
        });
        ...
        langChart
            ...
            .innerRadius(10)
            .dimension(langsDim)
            .group(langSumGroup) // by default, pie charts will use group.key as the label
            .title(function(d) {
    ...
    $scope.majDisplayartist = function(index) { $scope.displayartistIndex = index; };
    $scope.majFilterartist = function(index) { $scope.filterartistIndex = index; };
...
app.filter('projectsFilter', function() {
    return function(projects, scope) {
        ...
        if (scope.searchLang || scope.searchOrg || scope.filterartistIndex !== 0) {
            for (var i = 0; i < projects.length; i++) {
                var actProj = projects[i];

                switch (scope.filterartistIndex) {
                case 1:
                    filterartistBool = (actProj.watchers_count > 5) ? true : false;
                    break;
                case 2:
                    filterartistBool = (actProj.watchers_count > 30) ? true : false;
                    break;
                case 3:
                    filterartistBool = (actProj.watchers_count > 100) ? true : false;
                    break;
                case 4:
                    filterartistBool = (actProj.watchers_count > 300) ? true : false;
                    break;
                case 5:
                    filterartistBool = (actProj.watchers_count > 1000) ? true : false;
                    break;
                default:
                    filterartistBool = true;
                    break;
                }
